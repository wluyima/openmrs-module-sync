...
public class HibernateSyncInterceptor extends EmptyInterceptor implements ApplicationContextAware {
	...
	private void processPersistentCollection(AbstractPersistentCollection collection, Serializable key, String action, String originalRecordUuid,
		...
		try {
			...
			if (collMD != null) {
				...
				//If this is a simple type like Integer, serialization of the collection as below: 
				//<org.openmrs.Cohort>
				//	<memberIds type="java.util.Set(org.openmrs.Cohort)">[2, 3]</memberIds>
				//  ............. and more
				//Should work just fine as long as there is a Normalizer registered for it
				if (!OpenmrsObject.class.isAssignableFrom(elementClass) && SyncUtil.getNormalizer(elementClass) != null) {
					
					//Check if there is already sync item for the owner
					SyncItem syncItem = new SyncItem();
					...
					if (!ownerHasSyncItem) {
						...
						//create a sync item for the owner so that the collection changes get recorded along
	...
	/**
	 * Utility method that recursively fetches the CollectinMetadata for the specified collection
